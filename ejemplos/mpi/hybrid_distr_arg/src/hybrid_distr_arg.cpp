  ...
  if (MPI_Init(&argc, &argv) == MPI_SUCCESS) {
    ...
    if (argc == 3) {
      ...
      {  // NOLINT(whitespace/braces)
        ...
        #pragma omp critical(can_print) // impreesion en region critica
...
int calculate_start(int rank, int end, int workers, int begin) {
  // calcular el rango en base al begin porque ya no siempre es 0
