...
input_t* read_input() {
  ...
  /// settear valor por defecto
  input->next_unit = 0;
  queue_init(&input->queue);
  pthread_mutex_init(&input->can_access_next_unit, /* attr */ NULL);
  sem_init(&input->can_consume, /* pshared */ 0, /* value */ 0);
  pthread_mutex_init(&input->can_access_consumed_count, /* attr */ NULL);
  input->goldbach_numbers_index = 0;
  pthread_mutex_init(&input->can_add_goldbach_nunmber, NULL);
  input->goldbach_numbers_index = 0;
  input->goldbach_numbers = NULL;
