  ...
  , char* argv[]) {
    if (MPI_Init(&argc, &argv) == MPI_SUCCESS) {
        ...
        if (process_number != 0) {
            ...
            input->goldbach_numbers = (goldbach_number_t**)
              malloc(input->size * sizeof(goldbach_number_t*));
        ...
        MPI_Bcast(input->input_arr, input->size, MPI_INT64_T, 0
          , MPI_COMM_WORLD);
        ...
        #pragma omp parallel num_threads(thread_count) \
            default(none) shared(input, process_number, process_start \
            ...
            for (int iteration = process_start; iteration < process_finish
              ; ++iteration) {
                ...
                if (my_gold_number == NULL) {
                    fprintf(stderr
                      , "Error calculating Goldbach number on process %d,"
